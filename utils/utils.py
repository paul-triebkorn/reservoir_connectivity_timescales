...
def get_reservoir_timeconstants_input_mask(seed_reps, connectome, x_train, y_train, test_data_intact, test_data_shifted,
    ...
    for c, res_seed in enumerate(range(seed_reps)):
        ...
        # leakingRate=0.2 changing too fast, multiple with 0.05; changed
        ...
        # save time by not training the reservoir, as we only need the reservoir state, not the prediction
        # esn.fit(x_train, y_train, transientTime="Auto")
        # just create zero output layer
        ...
        # input_scaling = 0.1
        # esn._WInput[input_mask,:]  = input_scaling * len(input_mask)/np.sum(input_mask)
